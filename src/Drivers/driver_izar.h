...
private:
  float get_current_alarms(std::vector<unsigned char> &telegram) {
    float ret_val{};
    uint16_t alarms = 0;
  ...
  float get_previous_alarms(std::vector<unsigned char> &telegram) {
    float ret_val{};
    uint16_t alarms = 0;
  ...
  float get_remaining_battery_life_y(std::vector<unsigned char> &telegram) {
    float ret_val{};
    ret_val = (telegram[12] & 0x1F) / 2.0;
    return ret_val;
  };

  float get_transmit_period_s(std::vector<unsigned char> &telegram) {
    float ret_val{};
    ret_val = 1 << ((telegram[11] & 0x0F) + 2);
    return ret_val;
  };

  float get_total_water_m3(uint8_t *decrypted) {
    float ret_val{};
    ret_val = (this->uintFromBytesLittleEndian(decrypted + 1)) / 1000.0;
    return ret_val;
  };

  float get_last_month_total_water_m3(uint8_t *decrypted) {
    float ret_val{};
    ret_val = (this->uintFromBytesLittleEndian(decrypted + 5)) / 1000.0;
    return ret_val;
  };

  float get_current_month_total_water_l(uint8_t *decrypted) {
    float ret_val{};
